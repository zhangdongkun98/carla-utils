...
from ..trajectory import BaseCurve, BaseDiscreteTrajectory
...
class GeoVector3(object):
    def carla_vector_3d(vector: carla.Vector3D):
        vector3 = Vector3()
        vector3.x = vector.x
        vector3.y = vector.y
        vector3.z = vector.z
...
class GeoPose(object):
    @staticmethod
    def carla_transform(transform: carla.Transform):
        pose = Pose()
...
class NavPath(object):
    ...
    @staticmethod
    def cua_trajectory(header: Header, base_trajectory: BaseDiscreteTrajectory):
        trajectory = Path()
        trajectory.header = header
        trajectory.poses = [GeoPoseStamped.cua_state(header, base_trajectory.states(i)) for i in range(len(base_trajectory))]
        return trajectory
