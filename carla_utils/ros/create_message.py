...
def SensorsTransform(frame_id, timestamp, sensors):
    tfmsg = TFMessage()
    header = convert.header(frame_id, timestamp)
    for sensor in sensors:
        child_frame_id = sensor_frame_id(sensor)
        tfmsg.transforms.append(convert.GeoTransformStamped.carla_transform(header, child_frame_id, sensor.get_transform()))
    return tfmsg
