...
class BaseDiscreteTrajectory(object):
    """
        Currently, do not support batch.
    """
    
    def __init__(self, trajectory_state: State):
        self.t = trajectory_state.t
        self.x, self.y = trajectory_state.x, trajectory_state.y
        self.theta = trajectory_state.theta
        self.v, self.a = trajectory_state.v, trajectory_state.a
        self.k = trajectory_state.k
        return
    ...
    def query(self, t):
        index = np.abs(t - self.t).argmin()
        return State(
            t=self.t[index],
            x=self.x[index], y=self.y[index],
            theta=self.theta[index],
            v=self.v[index], a=self.a[index],
            k=self.k[index],
        )
