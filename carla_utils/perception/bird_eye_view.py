...
from .town_map_image import ColorLib
...
class BirdEyeView(object):
    ...
    def run_step(self, agent: BaseAgent, obstacles: List[BaseAgentObstacle]):
        ...
        cv2.fillPoly(image_bev, pts=[boundary_ego.astype(np.int64)], color=ColorLib.COLOR_CHAMELEON_0)
        ...
        for obs in obstacles:
            ...
            cv2.fillPoly(image_bev, pts=[boundary_obs.astype(np.int64)], color=ColorLib.COLOR_CHOCOLATE_1)
...
class BridEyeViewChannelWise(BirdEyeView):
    ...
    def run_step(self, agent: BaseAgent, obstacles: List[BaseAgentObstacle]):
        ...
        pts = (np.array(corners) - self.offset) / self.resolution
        cv2.polylines(image_route, [pts.astype(np.int64)], False, ColorLib.COLOR_WHITE, 20)
        ...
        cv2.fillPoly(image_ego, pts=[boundary_ego.astype(np.int64)], color=ColorLib.COLOR_WHITE)
        ...
        for obs in obstacles:
            ...
            vertices_obs = d2_local(obs, agent)
            boundary_obs = (vertices_obs - self.offset) / self.resolution
            cv2.fillPoly(image_obs, pts=[boundary_obs.astype(np.int64)], color=ColorLib.COLOR_WHITE)
