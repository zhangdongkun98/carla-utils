...
class BaseAgent(AgentABC):
    ...
    def get_current_a(self):
        return vector3DNorm(self.vehicle.get_acceleration())
    def get_state(self):
        current_transform = self.get_transform()
        current_v = self.get_current_v()
        current_a = self.get_current_a()
        return cvt.CuaState.carla_transform(current_transform, v=current_v, a=current_a)
