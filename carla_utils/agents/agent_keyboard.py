...
class KeyboardAgent(BaseAgent):
    def __init__(self, config, vehicle, sensors_master, global_path):
        ...
        if config.real:
            self.get_target = self.get_target_real
    ...
    def get_control(self, target):
        ...
        self.global_path.step_coverage(current_transform)
        control = target
        return control
    ...
    def get_target_real(self, _):
        self.pygame_clock.tick_busy_loop(0)
        control = self.kb_control.parse_events_real(self.pygame_clock)
        self.hud.tick(self.pygame_clock)
        pygame.display.flip()
        target = control
        return target
...
class KeyboardControl(object):
    ...
    def parse_events_real(self, clock):
        current_lights = self._lights
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                raise KeyboardInterrupt('quit')
