...
def createSensorListMaster(config, vehicle, sensors_param_list):
    core = config.core
    client, world = core.client, core.world
    blueprint_library = world.get_blueprint_library()
    for param in sensors_param_list:
        if not param.__contains__('sensor_tick'):
            param['sensor_tick'] = 1/ config.decision_frequency
    batch = [create_sensor_command(core, vehicle, blueprint_library, param) for param in sensors_param_list]
    sensor_ids = []
    ...
    for sensor, param in zip(sensors, sensors_param_list):
        transform, callback = param['transform'], param.get('callback', None)
        sensors_master.append(sensor, transform, callback)
    return sensors_master


def createSensorListMasters(config, vehicles, sensors_param_lists):
    core = config.core
    client, world = core.client, core.world
    blueprint_library = world.get_blueprint_library()
    ...
    for vehicle, sensors_param_list in zip(vehicles, sensors_param_lists):
        for param in sensors_param_list:
            if not param.__contains__('sensor_tick'):
                param['sensor_tick'] = 1/ config.decision_frequency
        batch.extend([create_sensor_command(core, vehicle, blueprint_library, param) for param in sensors_param_list])
...
class CarlaSensorMaster(object):
    def __init__(self, sensor: carla.Sensor, transform, callback):
        self.sensor = sensor
